...
```{r libraries, eval = TRUE, warning=FALSE, echo=FALSE}
library(MASS)
library(tidyverse)
library(boot)
library(tidyverse)
...
Variables: crim, rm, zn, chas, ptratio, medv

Split the file into half training and half validation
...
sapply(mod_formula_seq, function(form){
  mod <- as.formula(form)
  mean(sapply(1:K, function(j){
    fit <- glm(mod, data = Default[fold_ind != j,], family = "binomial")
...
sample(c("red","black"), size = 2, replace = TRUE)
sample(c("red","black"), size = 2, replace = TRUE)
sample(c("red","black"), size = 2, replace = FALSE)
```
...
x1 <- x[,1]
x_boot <- sapply(1:reps, function(j)sample(x1, size = n, replace = T))
...
ggplot(boot_dat) +
  geom_density(mapping = aes(x = mean, color = type))
```
